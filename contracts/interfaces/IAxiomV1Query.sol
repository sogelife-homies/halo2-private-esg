...
interface IAxiomV1Query {
    ...
    function queries(bytes32 keccakQueryResponse)
        external
        view
    ...
    event PoseidonResultEvent(
        bytes32 poseidonBlockResponse, bytes32 poseidonAccountResponse, bytes32 poseidonStorageResponse
    );
    ...
    event QueryInitiatedOnchain(
        bytes32 keccakQueryResponse, uint256 payment, uint32 deadlineBlockNumber, address refundee, bytes32 queryHash
    );
    ...
    function verifyResultVsMMR(uint32 mmrIdx, RecentMMRWitness calldata mmrWitness, bytes calldata proof) external;
