...
contract KPIStore is OwnableUpgradeable {
    ...
    function addPublicKPI(address company, uint256 kpiId, uint256 value) external {
        bytes32 kpiKey = keccak256(abi.encodePacked(company, kpiId));
    ...
    function addPrivateKPI(address company, uint256 kpiId, bytes32 commitment, bytes calldata proof) external {
        ...
        (bool success,) = privateKPIVerifierAddress.call(_encodeStratProof(proof));
        if (!success) {
            revert("SnarkVerificationFailed");
        }
        uint256 provenStat = uint256(bytes32(proof[0:32]));
        privateKPIStatistics[kpiKey] = uint256;
    }
}
...
//     function initialize(SnarkedVaultParams memory _params) public initializer {
...
//             poolAddress.collect(address(this), tickLower, tickUpper, type(uint128).max, type(uint128).max);
...
//     function _encodeStratProof(bytes calldata stratProf, IAxiomV1Query.StorageResponse[] calldata storageResponses)
//         internal
...
//         (bool success,) = stratVerfifierAddress.call(_encodeStratProof(stratProof, axiomResponse.storageResponses));
//         if (!success) {
//             revert("SnarkVerificationFailed");
//         }
...
//             TickMath.getTickAtSqrtRatio(uint160(uint256(BytesLib.toBytes32(stratProof, 0)))), tickSpacing
//         );
//         int24 upperBound = _applyTickSpacing(
//             TickMath.getTickAtSqrtRatio(uint160(uint256(BytesLib.toBytes32(stratProof, 32)))), tickSpacing
//         );
