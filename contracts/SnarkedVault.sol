...
contract AlphaProVault is IUniswapV3MintCallback, IUniswapV3SwapCallback, ERC20, ReentrancyGuard {
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        (uint256 fullAmount0, uint256 fullAmount1) = _burnLiquidityShare(fullLower, fullUpper, shares, totalSupply);
    ...
    function getBalance0() public view override returns (uint256) {
        return token0.balanceOf(address(this)).sub(accruedProtocolFees0).sub(accruedManagerFees0);
    }
    ...
    function getBalance1() public view override returns (uint256) {
        return token1.balanceOf(address(this)).sub(accruedProtocolFees1).sub(accruedManagerFees1);
    }
