...
//     // load the input
//     let bytes = ctx.assign_witnesses(input.x.iter().map(|b| F::from_bytes_le(b)));
...
//     let out_expected = keccak256(input.x.into_iter().flatten().collect::<Vec<u8>>());
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    //     x: vec![
    //         vec![0x40, 00, 00, 00, 00, 00, 00, 00],
    //         vec![0x4c, 0xcc, 0xcc, 0xcc, 0xcc, 0xcc, 0xcc, 00],
    //         vec![0x59, 0x99, 0x99, 0x99, 0x99, 0x99, 0x98, 00],
    //         vec![0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x68, 00],
    //     ],
    // };
